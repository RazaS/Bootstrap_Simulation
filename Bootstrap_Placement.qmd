...
![](images/clipboard-1750255435.png){width="582"}

#### Methods

![](images/clipboard-2829746797.png){width="444"}
...
The likelihood interval based on the likelihood ratio uses the negative log-likelihood function: $$
  \ell(\mu) = -\sum_{i=1}^n \log \left( f(x_i; \mu, \sigma) \right)
  $$

Where ( f(x_i; \mu, \sigma) ) is the normal PDF:

$$
  f(x_i; \mu, \sigma) = \frac{1}{\sqrt{2\pi}\sigma} \exp\left( -\frac{(x_i - \mu)^2}{2\sigma^2} \right)
  $$

The MLE for ( \mu ) minimizes ( \ell(\mu) ):
